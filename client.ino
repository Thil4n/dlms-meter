  ...
  unsigned char eop)
{
  ...
  do
  {
    ...
    if (available != 0)
    {
      ...
      Serial1.readBytes((char*) (frameData.data + frameData.size), available);
...
int com_readAllObjects(const char* invocationCounter)
{
  ...
  */
  ...
  // gxData ldn;
  // cosem_init(BASE(ldn), DLMS_OBJECT_TYPE_DATA, "1.0.0.9.2.255");
  // com_read(BASE(ldn), 2);
  // obj_toString(BASE(ldn), &data);
  // GXTRACE(GET_STR_FROM_EEPROM("xxx"), data);
  // obj_clear(BASE(ldn));
  // free(data);
  // //Read clock
  //gxClock clock1;
...
// gxRegister reg;
// cosem_init(BASE(reg), DLMS_OBJECT_TYPE_REGISTER, "1.0.0.6.2.255");
// com_read(BASE(reg),3);
//   com_read(BASE(reg), 2);

//   obj_toString(BASE(reg), &data);
...
//   free(data);
   ...
   // GXTRACE(GET_STR_FROM_EEPROM("BATTERY VOLTAGE s"), data);
    //obj_clear(BASE(bv));


 // Print the data of the gxRegister struct
  // Serial.println("Data of gxRegister struct:");
  //Serial.print("Base: ");
 // Serial.println(bv.base);
  //Serial.print("Value: ");
  //Serial.println(bv.value);
  // Serial.print("Scaler: ");
  // Serial.println(bv.scaler);
  // Serial.print("Unit: ");
  // Serial.println(bv.unit);
  // Serial.print("UnitRead: ");
  // Serial.println(bv.unitRead);
...
// int retttt = cosem_init2(BASE(dc), DLMS_OBJECT_TYPE_DISCONNECT_CONTROL, "0.0.96.3.10.255");

// retttt = com_method( BASE(dc), 1);


    dlmsVARIANT methodParams;
    var_init(&methodParams); // Initialize the dlmsVARIANT object

    // Populate the methodParams with the appropriate method parameters

    // Example: Call a method on the gxDisconnectControl object
    gxDisconnectControl disconnectCtrl;
    ret = cosem_init2(BASE(disconnectCtrl), DLMS_OBJECT_TYPE_DISCONNECT_CONTROL, "0.0.96.3.10.255");

      ret = com_method(BASE(disconnectCtrl), 4, &methodParams);
      if (ret == 0) {
        // Method call succeeded
        // Process the method response if needed
      } else {
        // Method call failed, handle the error
      }
...
//  print_dlmsVARIANT(bv.value);

// signed char data; // Assuming the data type of the register is 'long'.
...
void setup() {
  ...
  */
  ...
  //    ret = com_readAllObjects("1.0.0.9.5.255");
